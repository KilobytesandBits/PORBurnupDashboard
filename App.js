...
Ext.define('CustomApp',
		{
						...
						if (burnupChart !== null) {
							...
							Ext.Array.each(this.piRecords, function(piRecord, index) {
								...
								if (piBurnupChart !== null) {
									/*
									 * if(index == 0) { chartDiag.add({title :
									 * 'Scroll down for feature level burnup'}); }
									 */
			...
			_getChart : function(isMileStone, piRecord, chartDiag) {
				...
				/*
				 * var chartStartDate =
				 * _.isEmpty(_.compact(_.invoke(that.selectedMilestoneObj,
				 * 'get', 'ActiveStartDate'))) ?
				 * _.min(_.compact(_.invoke(that.piRecords, 'get',
				 * 'ActualStartDate'))) :
				 * _.first(_.compact(_.invoke(that.selectedMilestoneObj, 'get',
				 * 'ActiveStartDate'))); var chartEndDate =
				 * _.first(_.compact(_.invoke(that.selectedMilestoneObj, 'get',
				 * 'TargetDate')));
				 */
			...
			_getMilestoneProjectWiseView : function(milestoneRec, projWiseStatusDiag) {
				var projWiseStatusDiagId = projWiseStatusDiag.getId();
				...
				if (milestoneRec !== null) {
					...
					this._loadProjectWiseViewArtifacts(projWiseStatusDiag);
				}
			},
