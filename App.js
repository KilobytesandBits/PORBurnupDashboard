...
Ext.define('CustomApp',
		{
			...
			launch : function() {
				...
				var excludeCompleteMilestoneFilter = {
					xtype : 'rallycheckboxfield',
					id : 'excludeCompleteMilestoneCheckbox',
					boxLabel : 'Exclude All Completed Milestone',
					labelWidth : 200,
					padding : '0, 5, 10, 5',
					checked : false,
					listeners: {
						change: function( field, newValue, oldValue, eOpts ){
							this.excludeCompletedMSFilterSetting = newValue;
						},
						scope: this
					}
				};
				...
				var checkBoxFilterPanel = {
					...
					items:[executiveVisibityFilter, excludeCompleteMilestoneFilter]
				};
				
				var teamVSFilterPanel = {
					xtype: 'panel',
					width: 450,
					...
					items:[valuestreamFilter, teamFilter]
				};
				
				var milestoneFilterPanel = {
					xtype: 'panel',
					width: 700,
					border: false,
					layout : {
					...
					items:[checkBoxFilterPanel, teamVSFilterPanel]
				};
				...
				var toDateFilter = {
				        ...
				        listeners: {
				        	expand: function(field, eOpts){
				        		var value = this.down('datefield[name=from_date]').getValue();
				        		console.log('From date value: ', value);
				        		
				        		var today = new Date();
								var priorDate = Rally.util.DateTime.add(today, 'day', -15);
								console.log('Default start ToDate: ', priorDate);
								
			                	field.setMinValue(Ext.isDate(value) ? value : priorDate);
				        	},
				        	scope: this
				        }
				    };
				
				var targetDateFilterPanel = {
				    xtype: 'panel',
				    width: 300,
				    bodyPadding: 10,
				    title: 'Target Dates',
				    items: [fromDateFilter, toDateFilter]
				};
				...
				var filterButton = {
					...
					width : 50,
					handler : function() {
						if(this._isVSFilterValueSelected())
							this._loadAllMilestones();
						else
							this._displayMessageForVSSelection();
					},
					scope : this
				};
				...
				var filterPanelContaier = {
					...
					items : [ milestoneFilterPanel, targetDateFilterPanel, filterButton ]
				};
			...
			_isVSFilterValueSelected: function(){
				return (this.selectedMultiVSFilter !== null && this.selectedMultiVSFilter !== undefined 
				&& this.selectedMultiVSFilter.length > 0 && this.selectedMultiVSFilter !== '-- No Entry --');
			},
			
			_displayMessageForVSSelection: function(){
				
				var filterMsg = this._getFilterNoticeMessage() + '<div><p style="color:red">Valustream filter value NOT selected.</p></div>';
				Ext.getCmp('filterMessage').update(filterMsg);
			},
			
			_getFilterNoticeMessage: function(){
				var filterNoticeMsg = "<div><ul><li>Click on the <b>Filter</b> button to load your desired Milestones.</li><li>Select at least <u>one</u> <b>Valuestreams</b>" + 
									" to populate Milestones. Valuestreams filter selection is <u>mandatory</u>.</li> " + 
									"<li>By Default <b>Exclude All Completed Milestone</b> is turned OFF. Turned it ON in case you want to exclude the same. </li>" + 
									"<li>By default all Milestones with Target Date <b>15 days past Today</b>" + 
									" will be populate unless you specify complete Date range.</li><li>If not specified, Milestones belonging to <u>all</u> " + 
									"applicable Teams will be displayed.</li></ul></div>";
				return filterNoticeMsg;
			},
			...
			_setFilterMessage : function(isDefaultValues) {
				...
				var vsNameValues = (vsValues !== null && vsValues !== undefined && vsValues !== '-- No Entry --') ? vsValues : 'No Values Streams selected';
				var teamNameValues = (teamRefValues !== null && teamRefValues !== undefined) ? this._getProjectName(teamRefValues) : 'No Teams selected';
				var filterNoticeMsg =this._getFilterNoticeMessage();
				var filterMsg = filterNoticeMsg + "<div>Determing Milestones for ValueStream(s): <b>" + vsNameValues + "</b> and Team(s): <b>" + teamNameValues + "</b></div>";
				Ext.getCmp('filterMessage').update(filterMsg);
			},
			...
			_setDefaultFilteringValues : function() {
				...
				case "arpan.bandyopadhyay@lexmark.com":
					...
					this.excludeCompletedMSFilterSetting = true;
					break;
				...
				default:
					...
					this.excludeCompletedMSFilterSetting = false;
					//this.defaultValueStreamSetting = "FPA,Integration,Healthcare";
					//this.defaultTeamFilterSetting = "/project/3874483234,/project/7418464221,/project/9618552669,/project/2508024112,/project/3574043643,/project/2720145837";
				}
			},
			...
			_getValueStreamString : function(records) {
				...
				Ext.Array.each(records, function(vsRec) {
					if (vsRec.get('name') !== null && vsRec.get('name') !== '-- No Entry --')
					...
					columns : [ {
						...
						renderer : function(value, style, item, rowIndex) {
							...
							if (link.search('valuestream:') === -1) {
								...
								var formattedId = item.get('FormattedID');
								var lable = formattedId !== null ? Ext.String.format("<b>{0}-{1}</b>", formattedId, value) : Ext.String.format("<b>{0}</b>", value);
								link = Ext.String.format("<a target='_top' href='{1}'><b>{0}</b></a>", lable, Rally.nav.Manager.getDetailUrl(ref));
							} else {
			...
			_isMilestoneValidForDisplay : function(milestoneRec, milestoneComputeResult) {
				...
				if(this.excludeCompletedMSFilterSetting){
			...
			_getExcludeCompletedMilestoneFilter : function() {
				var excludeCompletedMSCheckBox = Ext.getCmp('excludeCompleteMilestoneCheckbox');
				return excludeCompletedMSCheckBox.getValue();
				// return true;
			},
			...
			_createMilestoneStoreFilter : function() {
				...
				var fromDate = Ext.getCmp('from_date').getValue();
				var toDate = Ext.getCmp('to_date').getValue(); 
				console.log('From Date: ', fromDate);
				console.log('To Date: ', toDate);
				
				//if Target date filters are provided add this filters.
				if(fromDate !== null){
				...
				var vsSelectedValues = this.selectedMultiVSFilter;
				
				if (this.isDefaultUserSettingPresent && this.defaultValueStreamSetting !== null && this.defaultValueStreamSetting !== undefined && this.defaultValueStreamSetting !== '') {
					vsSelectedValues = this.defaultValueStreamSetting;
				}

				var vsFilters = vsSelectedValues.split(',');
				
				var valueStreamMilestoneFilter = Ext.create('Rally.data.wsapi.Filter', {
					property : 'c_ValueStream',
					operator : 'contains',
					value : vsFilters[0]
				});
				...
				if (vsFilters.length > 1) {
					for (var i = 1; i < vsFilters.length; i++) {
						valueStreamMilestoneFilter = valueStreamMilestoneFilter.or(Ext.create('Rally.data.wsapi.Filter', {
							property : 'c_ValueStream',
							operator : 'contains',
							value : vsFilters[i]
						}));
				...
				this.projectMilestoneFilter = this.projectMilestoneFilter.and(valueStreamMilestoneFilter);
